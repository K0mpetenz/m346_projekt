...
aws ec2 create-security-group \
    --group-name "$NC_SECURITY_GROUP" \
    --description "$NC_INSTANCE_NAME"
...
aws ec2 run-instances \
    ...
    --key-name "$NC_KEY_NAME" \
    --security-groups "$NC_SECURITY_GROUP" \
    --iam-instance-profile Name=LabInstanceProfile \
    --user-data file://nc_ini.txt \
    --tag-specifications "ResourceType=instance,Tags=[{Key=Name,Value=$NC_INSTANCE_NAME}]" > /dev/null

# change the permissions for the ssh key to 600
chmod 600 ~/.ssh/"$NC_KEY_NAME".pem

# get the nextcloud instance id
NC_INSTANCE_ID=$(aws ec2 describe-instances \
    --filters "Name=tag:Name,Values=$NC_INSTANCE_NAME" \
...
aws ec2 run-instances \
    ...
    --instance-type t2.micro \
    --key-name "$DB_KEY_NAME" \
    --security-groups "$DB_SECURITY_GROUP" \
    --iam-instance-profile Name=LabInstanceProfile \
    --user-data file://db_ini.txt \
    --tag-specifications "ResourceType=instance,Tags=[{Key=Name,Value=$DB_INSTANCE_NAME}]" > /dev/null
